...
public:
	Particle(Vector3 pos, float mass, Vector3 vel = Vector3(0.0f), physx::PxGeometryType::Enum form = physx::PxGeometryType::eSPHERE, Vector4 color = Vector4(255, 255, 255, 1), Vector3 gravity = values::gravity, float damping = values::damping, double life_time = -1, BoundingBox const& pos_limits = BoundingBox());
	Particle(Vector3 pos, float mass, double life_time, Vector4 color = Vector4(1.0f));
