...
RigidBody::RigidBody(physx::PxPhysics* gPhysics, physx::PxScene* mScene, Vector3 pos, physx::PxActorType::Enum type, physx::PxShape* shape, double density, Vector4 color, double life_time, BoundingBox const& pos_limits) :
...
Actor* RigidBody::clone() const {
	return new RigidBody(_gPhysics, _mScene, _rigidActor->getGlobalPose().p, _rigidActor->getType(), createRegularShape(_renderItem->shape->getGeometryType(), getSize()), _density, _renderItem->color, _initial_life_time, _pos_limits);
}
